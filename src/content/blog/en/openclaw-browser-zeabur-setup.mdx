...
```bash
env | grep -i browser
# OPENCLAW_SANDBOX_BROWSER_HOST=service-698xxxxxxxxxxxxxxxxxxxxx
...
```bash
curl -s http://service-698xxxxxxxxxxxxxxxxxxxxx:9222/json/version
```
...
```json
{
  "browser": {
    ...
    "profiles": {
      "remote": {
        "cdpUrl": "http://service-698xxxxxxxxxxxxxxxxxxxxx:9222"
      }
    }
  }
}
