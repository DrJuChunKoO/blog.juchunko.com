...
When the request matches patterns like "寫一篇文章" or "write code", the Worker:
1. **Immediately** responds to G2: *"🦞 收到！正在寫文章中... 完成後會傳到 Telegram 📱"*
...
"畫一隻龍蝦" triggers image generation. Since G2's 1-bit display can't show photos, the image goes straight to Telegram.
...
For long tasks, the "收到！完成後傳 Telegram" pattern works surprisingly well. You ask by voice, go about your day, and the result appears on your phone later.

## What's Next

This is v5 of the bridge. The roadmap:

1. **Phase 2: Full OpenClaw Gateway integration** — route complex tasks (calendar, email, web search) through the Gateway's full tool suite
2. **Phase 3: ClawHub Skill** — package this as a reusable skill so any OpenClaw user can connect their G2 glasses
3. **Custom domain** — move from `dab.workers.dev` to a proper domain
...
1. Deploy the [bridge worker](https://github.com/dAAAb/g2-bridge) to Cloudflare
2. Set your secrets: `ANTHROPIC_API_KEY`, `G2_TOKEN`, `GATEWAY_URL`, `GATEWAY_TOKEN`
3. In Even Hub → Add Agent → paste your Worker URL and token
4. Start talking 🦞🕶️
